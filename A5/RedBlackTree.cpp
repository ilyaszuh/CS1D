...
size_t RedBlackTree::size_of(const struct Node* curr) const
{
	if(curr == NULL)
		return 0;
	return 1 + size_of(curr->left) + size_of(curr->right);
}

int RedBlackTree::rank_of_helper(const struct Node* curr, int key) const
{
	if(curr == NULL)
		return 0;

	if(curr->key == key)
		return size_of(curr->left);
	else if(curr->key > key)
		return rank_of_helper(curr->left, key);
	else
		return 1 + size_of(curr->left) + rank_of_helper(curr->right, key);
}

/* Rank operation. */
size_t RedBlackTree::rankOf(int key) const
{
	return rank_of_helper(root, key);
}

int RedBlackTree::selectHelper(size_t n) const
{
	Node* curr = root;
	while(curr != NULL)
	{
		size_t leftSize = size_of(curr->left);
		if(n == leftSize)
			return curr->key;
		else if(n < leftSize)
			curr = curr->left;
	...
	throw runtime_error("Red-black tree does not contain at least n elements.");
}

/* Select operation. */
int RedBlackTree::select(size_t rank) const
{
	return selectHelper(rank);
}
